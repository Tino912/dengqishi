...
- **Godot 版**（`godot-lightknight/`）：Godot 4.7.2。起步于「光能不能被墙挡住」的
  ...
  **现在已经跑在前面**：8 把武器 / 19 技能 / 14 恩赐 / 肉鸽三选一 / 精英词缀 /
  第二关「无芯之暗」/ 三关三张风格各异的地图 / 武器随机元素 + 攻击发光，
  这些 **Web 版都没有**。
...
- **Godot 独占（唯一实现）**：8 武器/19 技能、恩赐三选一、精英词缀、
  ...
  **5 元素与全部状态效果、攻击发光（挥击灯/弹丸灯）、第三关「灯河渡口」与
  三图美术风格（court/quarry/river）、敌人身子登记表**。
...
## 一键验证脚本（tools/）
- `tools/godot-lightknight.sh` — Godot 版完整自检（**324/324 断言**，rc=0，
  连跑两遍 `shots/report.json` **逐字节相同**，基线 md5 `fa02dae73042b6983de1cab170691a19`）
...
- 提交历史：`7b95646`（基线）→ `c5e8ed2`（宝箱/背包/守灯人那轮）→ `64886d7`（清理 + .gitignore）
  ...
  → `bb81642`（记录布局随机化那轮的记忆）→ **`0b9e2f8`（武器元素 + 攻击发光 + 第三关「灯河渡口」+ 三图/敌人样貌各异，+92 断言，324/324）**。
...
## 武器元素 / 攻击发光 / 第三关与三图（2026-09-25 新增，+92 断言）

用户这一轮提四件事：①攻击与弹药发光 ②武器随机元素 ③第三张地图 ④三图与敌人样貌各异。

### 元素 = 「五种不同形状」，不是五个倍率
- 表在 `content.gd` 的 `ELEMENTS`；顺序常量 `ELEMENT_ORDER = [fire,frost,shock,venom,radiant]`。
  火=灼烧 3.0s/22%（短促密集）· 毒=6.0s/10% + 减速 0.62（绵长稀疏）· 冰=冻结 1.15s 完全不动
  （**连外力给的速度都被清掉**）· 雷=麻痹 0.42s + 连锁**一跳**（半径 260、40% 伤、`has_los`
  **隔墙不连**）· 光=对 `shadow` 系 ×1.30 + 命中额外 +0.35 连击（**只对暗影系**，是"对策"）。
- **随机源 `_elem_rng`**：种子 `hash([run_seed, "elem"])`，**故意不带 `level_index`**；
  结果存 `prog["welem"] = {武器id: 元素id}` → **挂在武器 id 上**（同词条那套）→
  **换手不丢、过关换地图不重摇**。允许重复（48 种子实测 48/48 有重复，故意的）。
- 基础攻击被染成元素色；**弹丸颜色是发射那一刻的快照**（之后换武器不追着变）。

### 攻击发光 = 接进 `LightRig` 的真光
- 挥击灯 key 固定 `"swing"`，位置沿刀锋扫（`reach` 随出招时相 1→0.30、`sweep` 跟 `facing`），
  半径**跟着武器 `range`**（长兵器的光更远）；弹丸灯 key `"proj:<pid>"`。
- 两盏都 `shadow_enabled = true`（**会被墙挡**，是"真光"不是叠加层亮斑）。
- **`FX_LIGHT_MAX = 6`**：弹丸雨不封顶会把帧率拖垮；断言"24 弹丸只点 6 盏"+回收归零。
- 像素实测：挥击同点 0.0783→0.2070（2.6×）；弹丸 0.0921→0.9348（10.1×）。

### 第三关「灯河渡口」+ 三图风格各异
- 三关：`court` 2400×1700 ambient 0.90 scale 1.0 seed 10711（Boss 噬灯者幼体）→
  `quarry` 2800×2000 0.955 1.18 20422（devourer）→
  **`river` 3000×2200 0.975 1.35 30733（Boss `lampdemon_shadow`，`shadow:true`）**。
  第三关有专属道具（灯河浮台/浮灯）与新敌人 `tidehusk`/`lanternjaw`（另有 `ashling`）。
- 推进 `L1→L2→L3→通关`，**第三关之后不再往前**（防越界断言）。
- **"风格各异"的断言写法**：逐关加载各拍一张，比**各自玩家屏幕位置周围**的裁剪块，差异 >25%
  （实测 41.8 / 35.2 / 30.9 %），**外加对照"同关重建两次 = 0.00%"**，
  再加一条不依赖像素的"调色板 5 项 ≥4 项互异"。⚠️ **别整屏比**：整屏大部分是黑的，实测只差 3%。

### 敌人样貌
- `Art.body_kind()` 一张登记表；10 种敌人 → **9 套身子**（噬灯者幼/成体共用；`shade` 是兜底）。
- 断言**不能写"每种画出来不一样"**（永远绿），要写
  **「除了「灯影」本身，没有敌人落回兜底造型」**——新敌人忘登记就落这里。

### 两个新踩的坑（写进 README 六.16/17）
1. **给敌人的位移别走速度通道**：链锁牵引原是 `e.vx += cos*pw`，被冻结（清速度→拽近 0）和
   毒减速（×0.62）吃掉 → 同一条断言因"这局抽到什么元素"红绿。改成 `yank` 一次位移 + 清速度。
2. **比"黑底画面"的差异必须先把黑裁掉**（见上）。

## 已知待办（未做）
- Godot 版**三关都有了**，但设计稿里"盲女被吃→化为力量"那一关仍未做
  （盲女目前只在第二关**同行**）；第三关是自创的「灯河渡口」。
- **元素没有调平衡**（倍率是初值）、**元素之间没有组合**（无"破冰/导电"这类交互）、
  没有抗性/免疫。
- ~~Godot 版无商店~~ → **已有守灯人商店**（买灯油/重铸/锤炼）；但仍**无升级/存档/传送**，
  局内成长 = 三选一 + 宝箱 + 词条 + 武器元素。无手柄触屏。
