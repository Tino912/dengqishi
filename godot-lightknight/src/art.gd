...
const TEXT_SIZE := {
	"hit": 30.0,      # 普通命中（初版 14 → 22 → 30）
	"crit": 40.0,     # 暴击（初版 19 → 28 → 40）—— 要比命中大得一眼分得出
	"dot": 22.0,      # 元素持续伤害（初版 12）—— 刷得密，比命中收一点，否则屏幕全是字
	"hurt": 30.0,     # 自己受伤（初版 16）—— 和命中同尺寸，但有衬底辉光 + 红色
	"heal": 24.0,     # 回血 / 吸血 / 噬影（初版 13 ~ 16）
	"block": 24.0,    # 格挡 / 护光（初版 13 ~ 14）
	"status": 22.0,   # 元素状态刻字（初版 13）
}
