...
## **这一次**挥击的判定几何（`World.swing_cone()` 的返回值：r / arc / half / inner）。
##
## ⚠️ 绘制层画范围线时读它，**不许自己按武器表再算一遍** —— 那正是本轮修的病：
## 原来的线是绘制层自己算的（`range * reach_mul() * 0.9`），判定是另一套，
## 于是"标出来的线"与"真能打到的范围"分成三个方向跑偏。
##
## 存在玩家身上而不是世界上的理由同 `cast_t`：普通攻击（range_mul = 1.0）与
## 连灯刺的每一刺（0.9）范围**不一样**，绘制必须知道"这一次是哪一种"。
var swing_cone := {}
