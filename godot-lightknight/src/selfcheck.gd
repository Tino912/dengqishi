...
func _section_fog_night() -> void:
	...
	#
	# ⚠️⚠️ 而且这一步必须**把雾关掉**再量。雾铺满全屏之后，全屏均值由雾决定 ——
	# 把地图压暗，屏幕几乎不动（`MIST_MAX = 1.0` 时实测 0.2925 → 0.2839，只差 0.0086）。那量到
	# 的是**雾**，不是夜色。这与 fog.gd 类头注释 ② 是同一条规矩：**量哪一层，就把别层冻住。**
	# （当前 `MIST_MAX = 0.45`，雾对夜色的遮蔽弱了，但这条口径仍然要保留 ——
	#   "夜色接没接上"这件事不该受雾浓这个可调参数的影响。）
	var amb0 := w.ambient
	...
	# 雾铺满之后这个差值很小 —— 说明夜色在观感上被雾盖住了一部分。
	# 那是雾浓这个参数的直接后果，不该让"夜色有接上"这条断言跟着失效，所以两者分开量。
