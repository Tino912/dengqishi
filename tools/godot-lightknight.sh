...
#
# 预期结果：358/358 断言通过，退出码 0。七个核心结论：
...
#   ⑦ **夜色与迷雾**：关雾全屏平均亮度 0.158、开雾 0.212（可见但仍是夜里）；
...
#      夜色那个旋钮**单独验过**：同一帧只把 world.ambient 拧到最暗，全屏均值 0.212 -> 0.173。
...
if fg:
    ...
    print(f"    全屏平均亮度　关雾 {px['mean_off']:.4f} → 开雾 {px['mean_on']:.4f}"
          f"　同一帧只拧暗 ambient {px['mean_on']:.4f} → {px['mean_darkest']:.4f}（夜色旋钮接上了）")
